...
function renderSelectedStocksInfo(stock1: Stock, stock2: Stock): void {
  ...
  [stock1, stock2].forEach((stock) => {
    ...
    symbol.textContent = `Company: ${stock.symbol}`;

    const name = document.createElement("li");
    name.textContent = `Name: ${stock.name}`;

    const sector = document.createElement("li");
    sector.textContent = `Sector : ${stock.sector}`;

    const currentPrice = document.createElement("li");
    currentPrice.textContent = `Current Price: ${formatPrice(stock.currentPrice, stock.currency)}`;

    const currency = document.createElement("li");
    currency.textContent = `Currency: ${stock.currency}`;

    details.appendChild(symbol);
    details.appendChild(name);
    details.appendChild(sector);
    details.appendChild(currentPrice);
  ...
  try {
    ...
    function update() {
      try {
        ...
        renderSelectedStocksInfo(stock1, stock2);
